...
export const ThemeSettingsPanel = ({ currentTheme, availableThemes }) => {
  ...
  const [logoPreviewUrl, setLogoPreviewUrl] = useState(currentTheme?.logoUrl || null);
  ...
  const handleSaveTheme = async () => {
    ...
    // Fix: Removed attempt to access 'data' property that doesn't exist
    if (result?.success) {
      toast.success('Theme settings saved successfully!');
      
      // Dispatch theme update event to ensure we update the company name everywhere
      ...
      const companyNameEvent = new CustomEvent('company-name-updated', {
        detail: {
          companyName: themeToSave.companyName
        }
      });
      window.dispatchEvent(companyNameEvent);
      
      // Also update localStorage directly
      localStorage.setItem('company-name', themeToSave.companyName);
      console.log("Updated company name in localStorage:", themeToSave.companyName);
    } else {
      toast.error('Failed to save theme settings.');
    }
  };

  const saveThemeSettings = async (theme) => {
    try {
      ...
      // Create new theme
      const { data: newThemeData, error } = await supabase
        .from('themes')
        .insert({
          name: theme.name,
          primary_color: theme.primaryColor,
      ...
      if (theme.isActive) {
        // Dispatch theme update event with the newly created theme data
        const themeEvent = new CustomEvent('app-theme-updated', {
          detail: {
            theme: newThemeData // Use the returned data from the database
          }
        });
        window.dispatchEvent(themeEvent);
  ...
  const handleSetActive = async (isActive) => {
    try {
    ...
    <Card>
      ...
      <CardContent className="grid gap-6">
        ...
        <div>
          ...
          <Select
            onValueChange={(value) => handleFontChange(availableFonts.find(font => font.value === value) || availableFonts[0])}
          >
