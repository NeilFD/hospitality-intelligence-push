...
import { sampleBeverageRecipes, menuCategories, allergenTypes } from "@/data/sample-recipe-data";
...
const BeverageBible: React.FC = () => {
  ...
  const fetchRecipes = async () => {
    try {
      ...
      const {
        data: recipesData,
        error: recipesError
      } = await supabase.from('recipes').select('*').eq('module_type', 'beverage');
      if (recipesError) {
        throw recipesError;
      }
  ...
  const saveRecipeToSupabase = async (recipe: Recipe, showToast: boolean = true) => {
    try {
      ...
      const recipeData = {
        ...
        module_type: recipe.moduleType || 'beverage',
      ...
      console.log("Recipe saved successfully with ID:", recipe.id);
  ...
  const handleSaveRecipe = async (recipe: Recipe) => {
    try {
      ...
      console.log("Refreshing recipes from database after save");
      await fetchRecipes();
  ...
  const DesktopSidebar = () => <div className={`border-r border-gray-200 bg-white transition-all duration-300 h-full overflow-auto relative ${sidebarMaximized ? 'w-120' : sidebarOpen ? 'w-80' : 'w-0 overflow-hidden'}`}>
    {sidebarOpen && (
      <div className="p-4">
        ...
        <RecipeFilters moduleType="beverage" categories={menuCategories} allergens={allergenTypes} filters={filters} onFilterChange={handleFilterChange} onLetterSelect={handleLetterSelect} selectedLetter={selectedLetter} />
      </div>
    )}
  </div>;
    ...
    <div className="flex-1 relative">
      <div className="container px-4 py-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold">Beverage Bible</h1>
            <p className="text-muted-foreground">Manage and explore beverage recipes</p>
          </div>
      ...
      {formOpen && <RecipeFormDialog open={formOpen} onClose={() => setFormOpen(false)} onSave={handleSaveRecipe} recipe={editingRecipe} moduleType="beverage" categories={menuCategories} allergens={allergenTypes} />}
