...
const CreateNoteForm: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex space-x-2 mb-4">
        ...
        <Button 
          type="button" 
          variant={noteType === 'gif' ? 'default' : 'outline'}
          size="sm" 
          onClick={() => setNoteType('gif')}
        >
      ...
      <Textarea 
        ...
        className="min-h-[100px] font-['Special_Elite']"
      />
      ...
      {(noteType === 'image' || noteType === 'voice' || noteType === 'gif') && (
        <>
          <input 
            ...
            accept={noteType === 'image' || noteType === 'gif' ? 'image/*' : 'audio/*'} 
            className="hidden" 
          />
        ...
        <div className="flex space-x-2">
          {NOTE_COLORS.map((color, index) => (
            <button
              key={color}
              type="button"
              className={`w-6 h-6 rounded-full ${color} ${index === colorIndex ? 'ring-2 ring-offset-2 ring-blue-500' : ''}`}
...
const TeamNoticeboard: React.FC = () => {
  ...
  const updateNoteMutation = useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: Partial<TeamNote> }) => 
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-2xl font-bold">Team Noticeboard</h2>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-gray-100 animate-pulse rounded-lg h-[200px] w-full"></div>
          ))}
        </div>
      ) : (
        <>
