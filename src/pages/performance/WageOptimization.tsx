...
const UK_WAGE_BENCHMARKS = {
  minimumWage: 10.42, // UK National Living Wage (2023)
  averageFOH: 11.50,  // Average Front of House
  averageKitchen: 13.25, // Average Kitchen Staff
...
export default function WageOptimization() {
  ...
  const ESTIMATED_HOURS = {
    foh: {
      weekday: 24, // e.g., 3 staff × 8 hours
      weekend: 40  // e.g., 5 staff × 8 hours
    },
    kitchen: {
      weekday: 32, // e.g., 4 staff × 8 hours
      weekend: 40  // e.g., 5 staff × 8 hours
    }
  };
  ...
  const calculateMetrics = (data: any[]) => {
    ...
    const validData = data.filter(day => 
      (day.fohWages > 0 || day.kitchenWages > 0) && day.totalRevenue > 0);
    ...
    // Calculate totals
    const totalWages = validData.reduce((sum, day) => 
      sum + day.fohWages + day.kitchenWages, 0);
    ...
    const averageWagePerHour = (totalFOHHours + totalKitchenHours) > 0 ? 
      totalWages / (totalFOHHours + totalKitchenHours) : 0;
    ...
    const bestPerformingDay = daysWithRatios.length > 0 ? 
      `${daysWithRatios[0].dayOfWeek} (${(daysWithRatios[0].ratio * 100).toFixed(1)}%)` : '';
    
    const worstPerformingDay = daysWithRatios.length > 0 ? 
      `${daysWithRatios[daysWithRatios.length - 1].dayOfWeek} (${(daysWithRatios[daysWithRatios.length - 1].ratio * 100).toFixed(1)}%)` : '';
  ...
  const calculateTrends = (data: any[]) => {
    ...
    const weeklyTrends = Object.keys(weeklyData).map(key => {
      ...
      return {
        ...
        wagePercentage: week.totalRevenue > 0 ? 
          (week.totalWages / week.totalRevenue) * 100 : 0,
        averageWagePerDay: week.days > 0 ? 
          week.totalWages / week.days : 0,
        wagePerCover: week.totalCovers > 0 ?
          week.totalWages / week.totalCovers : 0
      };
    });
  ...
  const analyzeDayTypes = (data: any[]) => {
    ...
    const dayTypeData = dayOrder.map((day, index) => {
      ...
      return {
        ...
        wagePercentage: analysis.totalRevenue > 0 ? 
          (analysis.totalWages / analysis.totalRevenue) * 100 : 0,
        averageWages: analysis.count > 0 ? 
          analysis.totalWages / analysis.count : 0,
        averageRevenue: analysis.count > 0 ?
          analysis.totalRevenue / analysis.count : 0,
        wagePerCover: analysis.totalCovers > 0 ?
          analysis.totalWages / analysis.totalCovers : 0,
        averageCovers: analysis.count > 0 ?
          analysis.totalCovers / analysis.count : 0,
  ...
  const calculateWagePerCover = (data: any[]) => {
    ...
    const wagePerCoverData = data
      .filter(day => day.totalCovers > 0)
      .map(day => {
  ...
  const generateSuggestions = (data: any[]) => {
    ...
    if (dayAnalysis.length > 0) {
      ...
      dayAnalysis.sort((a, b) => a.averageCovers !== 0 && b.averageCovers !== 0 ? 
        (b.totalWages / b.averageCovers) - (a.totalWages / a.averageCovers) : 0);
  ...
  return (
    <div className="container py-6 max-w-[1400px] mx-auto animate-fade-in">
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span 
                    className={`font-medium ${
                      metrics.fohWagePerHour > UK_WAGE_BENCHMARKS.averageFOH * 1.1 
                        ? 'text-red-500' 
                        : metrics.fohWagePerHour < UK_WAGE_BENCHMARKS.averageFOH * 0.9 
                          ? 'text-amber-500' 
                          : 'text-green-500'
                    }`}
                  >
                ...
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span 
                    className={`font-medium ${
                      metrics.kitchenWagePerHour > UK_WAGE_BENCHMARKS.averageKitchen * 1.1 
                        ? 'text-red-500' 
                        : metrics.kitchenWagePerHour < UK_WAGE_BENCHMARKS.averageKitchen * 0.9 
                          ? 'text-amber-500' 
                          : 'text-green-500'
                    }`}
                  >
                    ...
                    <ChartContainer config={CHART_CONFIG}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={wagePerCoverData}>
                          ...
                          <Legend />
                          <Bar 
                            dataKey="wagePerCover" 
                            name="Wage per Cover" 
                            fill={COLORS.totalWages}
                            radius={[4, 4, 0, 0]} 
                          />
                        </BarChart>
                      </ResponsiveContainer>
                    ...
                    <ChartContainer config={CHART_CONFIG}>
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={wagesData.map(day => ({
                          ...
                          wagePercentage: (day.foodRevenue || 0) + (day.bevRevenue || 0) > 0 ? 
                            (((day.fohWages || 0) + (day.kitchenWages || 0)) / ((day.foodRevenue || 0) + (day.bevRevenue || 0))) * 100 : 0
                        }))}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="date" />
                          <YAxis />
                          ...
                          <Line 
                            type="monotone" 
                            dataKey="wagePercentage" 
                            name="Wage %" 
                            stroke={COLORS.fohWages} 
                            activeDot={{ r: 8 }}
                            strokeWidth={2}
                          />
                          <Line 
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={dayTypeAnalysis}>
                          ...
                          <Bar 
                            ...
                            radius={[4, 4, 0, 0]}
                          >
                            {dayTypeAnalysis.map((entry, index) => (
                              <Cell 
                                key={`cell-${index}`} 
                                fill={COLORS.dayOfWeek[index % COLORS.dayOfWeek.length]}
                              />
                            ))}
                          </Bar>
                        </BarChart>
                ...
                <Card className="lg:col-span-2">
                  ...
                  <CardContent>
                    <Table>
                      ...
                      <TableBody>
                        {dayTypeAnalysis.map((day) => (
                          <TableRow key={day.dayName} className={day.isWeekend ? 'bg-muted/30' : ''}>
                    ...
                    <ChartContainer config={CHART_CONFIG}>
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            data={[
                              { name: 'FOH', value: wagesData.reduce((sum, day) => sum + (day.fohWages || 0), 0) },
                              { name: 'Kitchen', value: wagesData.reduce((sum, day) => sum + (day.kitchenWages || 0), 0) }
                            ]}
                            cx="50%"
                            cy="50%"
                            labelLine={false}
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="h-2 bg-gray-200 rounded-full">
                          <div 
                            className={`h-2 rounded-full ${metrics.wagesPercentage <= 30 
                              ? 'bg-green-500' 
                              : metrics.wagesPercentage <= 35 
                                ? 'bg-yellow-500' 
                                : 'bg-red-500'}`}
                            style={{ width: `${Math.min(Math.max(metrics.wagesPercentage, 20), 50)}%` }}
                          ></div>
                        </div>
                        ...
                        <Table>
                          ...
                          <TableBody>
                            <TableRow>
                              ...
                              <TableCell 
                                className={metrics.fohWagePerHour > UK_WAGE_BENCHMARKS.averageFOH * 1.1 
                                  ? 'text-red-500' 
                                  : metrics.fohWagePerHour < UK_WAGE_BENCHMARKS.averageFOH * 0.9 
                                    ? 'text-amber-500' 
                                    : 'text-green-500'}
                              >
                                {metrics.fohWagePerHour > UK_WAGE_BENCHMARKS.averageFOH
                                  ? `+${formatCurrency(metrics.fohWagePerHour - UK_WAGE_BENCHMARKS.averageFOH)}/hr`
                                  : `-${formatCurrency(UK_WAGE_BENCHMARKS.averageFOH - metrics.fohWagePerHour)}/hr`}
                              </TableCell>
                            </TableRow>
                            <TableRow>
                              ...
                              <TableCell 
                                className={metrics.kitchenWagePerHour > UK_WAGE_BENCHMARKS.averageKitchen * 1.1 
                                  ? 'text-red-500' 
                                  : metrics.kitchenWagePerHour < UK_WAGE_BENCHMARKS.averageKitchen * 0.9 
                                    ? 'text-amber-500' 
                                    : 'text-green-500'}
                              >
                                {metrics.kitchenWagePerHour > UK_WAGE_BENCHMARKS.averageKitchen
                                  ? `+${formatCurrency(metrics.kitchenWagePerHour - UK_WAGE_BENCHMARKS.averageKitchen)}/hr`
                                  : `-${formatCurrency(UK_WAGE_BENCHMARKS.averageKitchen - metrics.kitchenWagePerHour)}/hr`}
                              </TableCell>
                            </TableRow>
                          </TableBody>
                        </Table>
