...
serve(async (req) => {
  ...
  try {
    ...
    // Instead of fetching and manipulating, use a simpler raw SQL approach
    const { data: result, error: sqlError } = await supabaseClient.rpc(
    ...
    if (sqlError) {
      console.error('Error updating message reaction via RPC:', sqlError);
      
      // Fallback to the previous approach if RPC fails
      ...
      const { data: updateData, error: updateError } = await supabaseClient
        ...
        .update({ 
          reactions: reactions,
          // Force an update to the updated_at timestamp to ensure the realtime subscription triggers
    ...
    console.log(`[add_message_reaction] RPC update successful:`, result);
